...
const sponseeHtml = \unction appendInlineGeneratedCardSponsee(threadId) {
        ...
        const key = threadId + '-mega-pipeline';
        let wrap = thread.querySelector('[data-inline-card=\"' + key + '\"]');
        let created = false;
        if (!wrap) {
...
const sponsorHtml = sponseeHtml.replace('appendInlineGeneratedCardSponsee', 'appendInlineGeneratedCardSponsor');

// Sponsee
let sponsee = fs.readFileSync('Sponsee/The Main Page/messages.html', 'utf8');
sponsee = sponsee.replace(/function appendInlineGeneratedCardSponsee\\(threadId, cardType\\) \\{[\\s\\S]*?return true;\\s*\\}/, sponseeHtml);
sponsee = sponsee.replace(/function updateQA\\(id\\) \\{\\s*const row = document\\.getElementById\\(\"qa-row\"\\);/, \unction updateQA(id) {
        appendInlineGeneratedCardSponsee(id);
        const row = document.getElementById("qa-row");\);
sponsee = sponsee.replace(/'<span style=\"font-size:12px;color:var\\(--grn-3\\);padding:4px 0;font-weight:700;\">.*?generate.*?<\\/button>';/g, \"''\");
fs.writeFileSync('Sponsee/The Main Page/messages.html', sponsee, 'utf8');

// Sponsor
let sponsor = fs.readFileSync('Sponsor/The Main Page/sponsor_messages.html', 'utf8');
sponsor = sponsor.replace(/function appendInlineGeneratedCardSponsor\\(threadId, cardType\\) \\{[\\s\\S]*?return true;\\s*\\}/, sponsorHtml);
sponsor = sponsor.replace(/function updateQA\\(id\\) \\{\\s*const row = document\\.getElementById\\(\"qa-row\"\\);/, \unction updateQA(id) {
        appendInlineGeneratedCardSponsor(id);
        const row = document.getElementById("qa-row");\);
sponsor = sponsor.replace(/'<span style=\"font-size:12px;color:var\\(--grn-3\\);padding:4px 0;font-weight:700;\">.*?generate.*?<\\/button>';/g, \"''\");
fs.writeFileSync('Sponsor/The Main Page/sponsor_messages.html', sponsor, 'utf8');

console.log('Replaced', sponsee.includes('mega-pipeline'), sponsor.includes('mega-pipeline'));
